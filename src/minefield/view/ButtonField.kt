...
import minefield.model.FieldEvent
...
class ButtonField(private val field: Field): JButton() {
    ...
    private fun applyStyle(field: Field, event: FieldEvent) {
        when (event) {
            ...
            FieldEvent.OPENING -> applyOpeningStyle()
            FieldEvent.MARKED -> applyMarkedStyle()
            else -> applyDefaultStyle()
        }

        // refresh assíncrono no button
    ...
    private fun applyOpeningStyle() {
        ...
        // estilo de acordo com o numero de minas dos vizinhos
        foreground = when (field.qtMinedNeighbors) {
            1 -> COLOR_TXT_GREEN
            2 -> Color.BLUE
            3 -> Color.YELLOW
            4, 5, 6 -> Color.RED
        ...
        text = if (field.qtMinedNeighbors > 0) field.qtMinedNeighbors.toString() else ""
    }
